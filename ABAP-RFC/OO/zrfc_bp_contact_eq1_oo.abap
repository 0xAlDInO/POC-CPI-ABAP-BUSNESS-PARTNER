*&---------------------------------------------------------------------*
*& Function Module : ZRFC_BP_CONTACT_EQ1 (Version POO)
*& Remote-Enabled : Yes (SE37 > Attributes > Remote-Enabled Module)
*&---------------------------------------------------------------------*
*& Requires includes LZFG_BP_CONTACT_OO_TOP and LZFG_BP_CONTACT_OO_F01.
...
*"----------------------------------------------------------------------
  ...
  DATA lo_handler TYPE REF TO lcl_bp_contact_handler.

  CREATE OBJECT lo_handler.

  " Délégation complète du traitement à l'instance de classe (Option A)
  lo_handler->execute(
    EXPORTING
